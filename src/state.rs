...
impl UtxoSet {
    ...
    pub fn genesis(a: u64, pk: &[u8]) -> Self {
        ...
        let tx = Transaction{version:1,inputs:vec![],outputs:vec![TxOutput{amount:a,public_key:pk.try_into().unwrap(),spendable_after:0}],ring_size:1,signatures:vec![]};
...
mod tests {
    ...
    fn out(v:&[u64],pk:&[u8])->Vec<TxOutput>{v.iter().map(|&a|TxOutput{amount:a,public_key:pk.try_into().unwrap(),spendable_after:0}).collect()}
