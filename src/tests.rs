...
fn integration_reward_proportionality() {
    ...
    let mut c1 = Commitment { miner_id: pk, access_ops_per_sec: 25_000_000., block_number: 0, total_access_ops: 25_000_000., time_seconds: 1., signature: vec![] };
    let msg1 = crate::commitment::commit_msg(&c1);
    c1.signature = sk.sign(&msg1).to_bytes().to_vec();
    let mut c2 = Commitment { miner_id: pk, access_ops_per_sec: 25_000_000., block_number: 0, total_access_ops: 25_000_000., time_seconds: 1., signature: vec![] };
    let msg2 = crate::commitment::commit_msg(&c2);
    c2.signature = sk.sign(&msg2).to_bytes().to_vec();
    let r = compute_block_rewards(20000, &[c1, c2], &[25_000_000.0], 25_000_000.0);
